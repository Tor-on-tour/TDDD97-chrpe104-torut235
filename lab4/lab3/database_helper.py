...
def create_connection():
    ...
    conn.row_factory = sqlite3.Row          #Ollehokuspokus, "annars far man ut en tuple, nu far man en dictionary" - Olle anno 2017
    cursor = conn.cursor()
    return cursor

def add_user(user_obj):
    cursor = create_connection()
    SQL_perform = 'INSERT INTO users VALUES(:token, :email, :salt, :hash, :firstname, :familyname, :gender, :city, :country)'
...
def change_password_db(user_email, salt, hashed):
    cursor = create_connection()
    cursor.execute('UPDATE users SET salt = ?, hash = ? WHERE email = ?', (salt, hashed, user_email))
    cursor.connection.commit()
